...
Triggers when an inventory owner sells an equipment.
...
# Usage
This node listens to the event where an equipment sale occurs between inventory owners via a shop.
The shop owner entity must be running a "Self Inventory Shop", not "Custom Items Shop".
The item sold must be an equipment, not a normal item.
...
when an equipment with the specific configuration ID is sold
