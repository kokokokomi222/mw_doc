...
Triggers when an inventory owner buys an equipment.
...
# Usage
This node listens to the event where an equipment purchase occurs between inventory owners via a shop.
The shop owner entity must be running a "Self Inventory Shop", not "Custom Items Shop".
The item purchased must be an equipment, not a normal item.
...
Note that "equipment ID" is an integer that identifies an instance of an equipment and it is typically small (e.g. `3`),
while "equipment configuration ID" is a configuration ID that identifies a class of an equipment and it is typically large (e.g. `1107296257`).

# Example
In this example, we print `"I bought the cool sword!"`
when an equipment with the specific configuration ID is purchased
